...
function begin_play()
{
	...
	{
		alert("Each of the players must enter a valid player name.");
		return;
	}
	...
	else
	{
		if(!started)
		{
			...
			this.started = true;
			document.getElementById("turn_info").innerHTML="Active Player: "+ whose_move();
		}
		else
...
function reset_play()
{
	...
	document.getElementById("turn_info").innerHTML="Welcome to Tic-Tac-Toe!";
...
function play() 
{
	...
	if (this.started)
	{
		var turn = document.getElementById('move_text_id').value;


		//check for valid entry against ids
		if (!this.table_ids.includes(turn)) 
			alert("Please enter a valid square (Remember! values are case-sensitive)");

		else 
		{
			if(this.board_state[this.table_ids.indexOf(turn)] == -1)
			{
				if (whose_move()) 
				{
					document.getElementById(turn).innerHTML = "X";
					this.board_state[this.table_ids.indexOf(turn)] = 1;
				...
				else 
				{
					...
					this.board_state[this.table_ids.indexOf(turn)] = 0;
					document.getElementById("turn_info").innerHTML="Player "+ 1 + " turn";
					//next turn
					toggle_move();
				}
				//game engine to check all possible table states
				logic();       
			}
 ...
 function logic()
 {
	...
	if(this
		.board_state[2] == 1 && this.board_state[5] == 1 && this.board_state[8] == 1)
	{
		alert("Player "+1+" won!!");
		reset_play();
	}
